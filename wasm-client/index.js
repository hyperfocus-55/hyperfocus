...
// import engine from '@hyperfocus/wasm-engine/build/bin.wasm';
// import * as cssHref from '@hyperfocus/wasm-client/client.css';
...
// console.log(Buffer);
// require('buffer');
console.log('these are the interfaces generated by the esm bindings', WasmInterface);

export const buildClient = async (options = {fps: 2}) => {
...
export const start = async (options) => {
  const wasm = await loadWasm(options.context);
  wasm.start(options.canvas);
  options.wasm = wasm;
  window.wasm = wasm;
  window.memory = wasm.memory;
  console.log('use window.wasm or window.memory to investigate the memory');
...
export const loadWasm = async (context, file = '/hyperfocus/wasm-engine/build/bin.wasm') => {
  const wasm = await WebAssembly
    .instantiateStreaming(fetch(file), {
      env: {
        ...
        'console.log': n => console.log('input from wasm-engine.log:', n, wasm.instance.exports.memory, liftString(wasm.instance.exports.memory, n)),
        drawRect: r => console.log('drawingRect invoked!', 'input from wasm-engine.drawRect', r) || context.fillRect(r.x, r.y, r.h, r.w)
      }});
